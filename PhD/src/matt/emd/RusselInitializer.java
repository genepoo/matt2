 ...
 */
public final class RusselInitializer implements Initializer {
    ...
    private final class Delta implements Comparable {
        ...
        public Delta(final int i, final int j) {
            mI = i; mJ = j;
        }
    ...
    /** Vector pointing to the minimum Delta-value in the corresponding row */    
    private Delta[] mMinDeltaPerRow;
                ...
                if (oldV != colHeader.getV()) //Recalculate the column in the deltaMatrix if v<sub>j</sub> really changed
                    for (i2 = 1; i2 <= mTableau.getM(); i2++)
                        if (!Float.isNaN(mTableau.getRowHeader(i2).getU())) {
                            ...
                            if (mMinDeltaPerRow[i2 - 1] == mDeltaMatrix[i2 - 1][j - 1]) { //MinDeltaValue must be changed
                                mMinDeltaPerRow[i2 - 1] = null;
                                for (j2 = 1; j2 <= mTableau.getN(); j2++)
                                    if (!Float.isNaN(mTableau.getColHeader(j2).getV()) && (mMinDeltaPerRow[i2 - 1] == null || mMinDeltaPerRow[i2 - 1].compareTo(mDeltaMatrix[i2 - 1][j2 - 1]) > 0))
                                        mMinDeltaPerRow[i2 - 1] = mDeltaMatrix[i2 - 1][j2 - 1];
                            }
                        }    
            }
        }
    ...
    private void removeColFromConsideration(int j) {
        ...
        int i, j2;
        float oldU;
        boolean minDeltaPerRowAlreadyCalced;
        for (i = 1; i <= mTableau.getM(); i++) { //Check rows for changes which have to be done
            TableauRowHeader rowHeader = mTableau.getRowHeader(i); minDeltaPerRowAlreadyCalced = false;
            if (!Float.isNaN(rowHeader.getU()) && mTableau.getCell(i, j).getCost() == rowHeader.getU()) { //c<sub>ij</sub> meight have been the only maximum-value in this row
                ...
                if (oldU != rowHeader.getU()) { //Recalculate the row in the deltaMatrix if u<sub>i</sub> really changed
                    mMinDeltaPerRow[i - 1] = null;
                    for (j2 = 1; j2 <= mTableau.getN(); j2++)
                        if (!Float.isNaN(mTableau.getColHeader(j2).getV())) {
                            mDeltaMatrix[i - 1][j2 - 1].Set(mTableau.getCell(i, j2).getCost(), rowHeader.getU(), mTableau.getColHeader(j2).getV());
                            if (mMinDeltaPerRow[i - 1] == null || mMinDeltaPerRow[i - 1].compareTo(mDeltaMatrix[i - 1][j2 - 1]) > 0)
                                mMinDeltaPerRow[i - 1] = mDeltaMatrix[i - 1][j2 - 1];
                        }
                   minDeltaPerRowAlreadyCalced = true;
                }
            }
            if (!minDeltaPerRowAlreadyCalced && mMinDeltaPerRow[i - 1] == mDeltaMatrix[i - 1][j - 1]) { //Find new Min
                mMinDeltaPerRow[i - 1] = null;
                for (j2 = 1; j2 <= mTableau.getN(); j2++)
                    if (!Float.isNaN(mTableau.getColHeader(j2).getV()) && (mMinDeltaPerRow[i - 1] == null || mMinDeltaPerRow[i - 1].compareTo(mDeltaMatrix[i - 1][j2 - 1]) > 0))
                        mMinDeltaPerRow[i - 1] = mDeltaMatrix[i - 1][j2 - 1];
            }
        }
    }
    ...
    private void initializeDeltaMatrix() {
        ...
        if (mDeltaMatrix == null || mDeltaMatrix.length != mTableau.getM() || mDeltaMatrix[0] == null || mDeltaMatrix[0].length != mTableau.getN()) {
            ...
            for (i = 1; i <= mTableau.getM(); i++) 
                for (j = 1; j <= mTableau.getN(); j++) 
                    mDeltaMatrix[i - 1][j - 1] = new Delta(i, j);
        }
        
        if (mMinDeltaPerRow == null || mMinDeltaPerRow.length != mTableau.getM()) {
            mMinDeltaPerRow = new Delta[mTableau.getM()];
        }
        ...
        for (i = 1; i <= mTableau.getM(); i++) {
            for (j = 1; j <= mTableau.getN(); j++) {
                ...
                if (mMinDeltaPerRow[i - 1] == null || mMinDeltaPerRow[i - 1].compareTo(mDeltaMatrix[i - 1][j - 1]) > 0)
                    mMinDeltaPerRow[i - 1] = mDeltaMatrix[i - 1][j - 1];
            }
        }
    }
    ...
    private boolean findBasicVariable() {
        ...
        Delta deltaMin = null;
        for (i = 1; i <= mTableau.getM(); i++)
            if (!Float.isNaN(mTableau.getRowHeader(i).getU()) && (deltaMin == null || deltaMin.compareTo(mMinDeltaPerRow[i - 1]) > 0))
                deltaMin = mMinDeltaPerRow[i - 1];
        if (deltaMin == null) //Nothing found!
            return false;
        addBasicVariable(deltaMin.getI(), deltaMin.getJ());
        return true;
    }
    
    /** Creates a new instance of RusselInitialisator */
    public RusselInitializer() {
        this(DL_NONE);
    }
    
    /** Creates a new instance of RusselInitialisator */
    public RusselInitializer(int debugLevel) {
