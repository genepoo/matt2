...
public class CapturePlayback extends JPanel implements ActionListener, ControlContext {
    ...
    public CapturePlayback() {
        ...
        waveB = addButton("Submit", buttonsPanel, false);
    ...
    public void saveToFile(AudioFileFormat.Type fileType) {
        ...
        try {
            ...
            int id = upload.submit(audioInputStream, fileType);
            String url = "" + applet.getDocumentBase();
            url = url.substring(0, url.lastIndexOf("/"));
            url += "/searching.php?id=" + id;
            System.out.println(url);
            applet.getAppletContext().showDocument(new URL(url), "resultsFrame");

        } catch (Exception ex) { reportStatus(ex.toString()); ex.printStackTrace();}
