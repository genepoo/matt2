...
public final class FastFourierTransform {
  ...
  public final float[] fftLogMag(float[] x) {      
      return fftLogMag(x, 0, x.length);    
  }
  
  
  public final float[] fftMag(float[] x, int start, int length) {
  ...
  public final float[] fftLogMag(float[] x, int start, int length) {
    // assume n is a power of 2
    n = length;
    nu = (int)(Math.log(n)/Math.log(2));
    int n2 = n/2;
    ...
    float tr, ti, p, arg, c, s;
    for (int i = 0; i < n; i++) {
      xre[i] = (float) Math.log((double) x[i + start]);
      xim[i] = 0.0f;
    }
    int k = 0;
  ...
  public static int smallestPowerOf2(int value)
  {
    ...
    public void printFft(float[] fft, float sampleRate)
    {
        float binWidth = fft.length / sampleRate;
        System.out.println("Frame size: " + fft.length);
        for (int i = 0 ; i < fft.length ; i ++)
        {
            System.out.println(((float) i) * binWidth + "\t" + fft[i]);
        }
    }
  ...
  public static void main(String[] args)
  {
      ...
      System.out.println(FastFourierTransform.smallestPowerOf2(i));
  }
}
